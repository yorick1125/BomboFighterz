...
export default class Sprite {
	...
	static generateArrowSprites() {
		...
		sprites.push(new Sprite(images.get(ImageName.Arrows), 0, 0, 24, 24));
		sprites.push(new Sprite(images.get(ImageName.Arrows), 24, 0, 24, 24));

		return sprites;
	}


	static generateWalkingSprites(spriteSheet, tileWidth, tileHeight) {
		const sprites = [];
		const sheetWidth = 3;
		const sheetHeight = 4;
